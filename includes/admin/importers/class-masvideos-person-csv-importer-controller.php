<?php
/**
 * Class MasVideos_Person_CSV_Importer_Controller file.
 *
...
/**
 * Person importer controller - handles file upload and forms in admin.
 *
 * @package     MasVideos/Admin/Importers
 * @version     1.0.0
 */
class MasVideos_Person_CSV_Importer_Controller {
    ...
    /**
     * Whether to skip existing persons.
     *
     * @var bool
     */
     ...
     * @return MasVideos_Person_CSV_Importer
     */
    public static function get_importer( $file, $args = array() ) {
        $importer_class = apply_filters( 'masvideos_person_csv_importer_class', 'MasVideos_Person_CSV_Importer' );
        $args           = apply_filters( 'masvideos_person_csv_importer_args', $args, $importer_class );
    ...
    public static function is_file_valid_csv( $file, $check_path = true ) {
        if ( $check_path && apply_filters( 'masvideos_person_csv_importer_check_import_file_path', true ) && false !== stripos( $file, '://' ) ) {
            return false;
        }
    ...
    protected static function get_valid_csv_filetypes() {
        return apply_filters(
            'masvideos_csv_person_import_valid_filetypes', array(
                'csv' => 'text/csv',
                'txt' => 'text/plain',
            )
        );
    ...
    public function __construct() {
        ...
        $this->steps = apply_filters( 'masvideos_person_csv_importer_steps', $default_steps );
        ...
        if ( $this->map_preferences ) {
            add_filter( 'masvideos_csv_person_import_mapped_columns', array( $this, 'auto_map_user_preferences' ), 9999 );
        }
    }
    ...
    public function handle_upload() {
        // phpcs:disable WordPress.CSRF.NonceVerification.NoNonceVerification -- Nonce already verified in MasVideos_Person_CSV_Importer_Controller::upload_form_handler()
        $file_url = isset( $_POST['file_url'] ) ? masvideos_clean( wp_unslash( $_POST['file_url'] ) ) : '';

        if ( empty( $file_url ) ) {
            if ( ! isset( $_FILES['import'] ) ) {
                return new WP_Error( 'masvideos_person_csv_importer_upload_file_empty', __( 'File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your php.ini or by post_max_size being defined as smaller than upload_max_filesize in php.ini.', 'masvideos' ) );
            }

            if ( ! self::is_file_valid_csv( masvideos_clean( wp_unslash( $_FILES['import']['name'] ) ), false ) ) {
                return new WP_Error( 'masvideos_person_csv_importer_upload_file_invalid', __( 'Invalid file type. The importer supports CSV and TXT file formats.', 'masvideos' ) );
            }
            ...
            if ( isset( $upload['error'] ) ) {
                return new WP_Error( 'masvideos_person_csv_importer_upload_error', $upload['error'] );
            }
        ...
        return new WP_Error( 'masvideos_person_csv_importer_upload_invalid_file', __( 'Please upload or provide the link to a valid CSV file.', 'masvideos' ) );
    }
    ...
    protected function mapping_form() {
        ...
        if ( empty( $sample ) ) {
            $this->add_error(
                ...
                array(
                    array(
                        'url'   => admin_url( 'edit.php?post_type=person&page=person_importer' ),
    ...
    public function import() {
        ...
        // phpcs:disable WordPress.CSRF.NonceVerification.NoNonceVerification -- Nonce already verified in MasVideos_Admin_Importers::do_ajax_person_import()
        if ( ! empty( $_POST['map_from'] ) && ! empty( $_POST['map_to'] ) ) {
            ...
            update_user_option( get_current_user_id(), 'masvideos_person_import_mapping', $mapping_to );
        } else {
        ...
        wp_localize_script(
            'masvideos-person-import', 'masvideos_person_import_params', array(
                'import_nonce'    => wp_create_nonce( 'masvideos-person-import' ),
                'mapping'         => array(
                    'from' => $mapping_from,
                    'to'   => $mapping_to,
                ),
        ...
        wp_enqueue_script( 'masvideos-person-import' );
    ...
    protected function done() {
        ...
        $imported   = isset( $_GET['persons-imported'] ) ? absint( $_GET['persons-imported'] ) : 0;
        $updated    = isset( $_GET['persons-updated'] ) ? absint( $_GET['persons-updated'] ) : 0;
        $failed     = isset( $_GET['persons-failed'] ) ? absint( $_GET['persons-failed'] ) : 0;
        $skipped    = isset( $_GET['persons-skipped'] ) ? absint( $_GET['persons-skipped'] ) : 0;
        $errors     = array_filter( (array) get_user_option( 'person_import_error_log' ) );
        // phpcs:enable
    ...
    protected function auto_map_columns( $raw_headers, $num_indexes = true ) {
        ...
        $default_columns = $this->normalize_columns_names(
            apply_filters(
                'masvideos_csv_person_import_mapping_default_columns', array(
                    __( 'ID', 'masvideos' )                        => 'id',
                    ...
                    __( 'Also known as', 'masvideos' )             => 'also_known_as',
                    __( 'Birth Place', 'masvideos' )               => 'place_of_birth',
                    __( 'Birth Date', 'masvideos' )                => 'birthday',
                    __( 'Death Date', 'masvideos' )                => 'deathday',
        ...
        $special_columns = $this->get_special_columns(
            $this->normalize_columns_names(
                apply_filters(
                    'masvideos_csv_person_import_mapping_special_columns',
                    array(
        ...
        return apply_filters( 'masvideos_csv_person_import_mapped_columns', $headers, $raw_headers );
    }
    ...
    public function auto_map_user_preferences( $headers ) {
        $mapping_preferences = get_user_option( 'masvideos_person_import_mapping' );
    ...
    protected function get_mapping_options( $item = '' ) {
        ...
        $options        = array(
            ...
            'also_known_as'          => __( 'Also known as', 'masvideos' ),
            'place_of_birth'         => __( 'Birth Place', 'masvideos' ),
            'birthday'               => __( 'Birth Date', 'masvideos' ),
            'deathday'               => __( 'Death Date', 'masvideos' ),
        ...
        return apply_filters( 'masvideos_csv_person_import_mapping_options', $options, $item );
    }
}
