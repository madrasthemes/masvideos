<?php
/**
 * Class MasVideos_Video_CSV_Importer_Controller file.
 *
...
/**
 * Video importer controller - handles file upload and forms in admin.
 *
 * @package     MasVideos/Admin/Importers
 * @version     1.0.0
 */
class MasVideos_Video_CSV_Importer_Controller {
    ...
    /**
     * Whether to skip existing videos.
     *
     * @var bool
     */
     ...
     * @return MasVideos_Video_CSV_Importer
     */
    public static function get_importer( $file, $args = array() ) {
        $importer_class = apply_filters( 'masvideos_video_csv_importer_class', 'MasVideos_Video_CSV_Importer' );
        $args           = apply_filters( 'masvideos_video_csv_importer_args', $args, $importer_class );
    ...
    public static function is_file_valid_csv( $file, $check_path = true ) {
        if ( $check_path && apply_filters( 'masvideos_video_csv_importer_check_import_file_path', true ) && false !== stripos( $file, '://' ) ) {
            return false;
        }
    ...
    protected static function get_valid_csv_filetypes() {
        return apply_filters(
            'masvideos_csv_video_import_valid_filetypes', array(
                'csv' => 'text/csv',
                'txt' => 'text/plain',
            )
        );
    ...
    public function __construct() {
        ...
        $this->steps = apply_filters( 'masvideos_video_csv_importer_steps', $default_steps );
        ...
        if ( $this->map_preferences ) {
            add_filter( 'masvideos_csv_video_import_mapped_columns', array( $this, 'auto_map_user_preferences' ), 9999 );
        }
    }
    ...
    public function handle_upload() {
        // phpcs:disable WordPress.CSRF.NonceVerification.NoNonceVerification -- Nonce already verified in MasVideos_Video_CSV_Importer_Controller::upload_form_handler()
        $file_url = isset( $_POST['file_url'] ) ? masvideos_clean( wp_unslash( $_POST['file_url'] ) ) : '';

        if ( empty( $file_url ) ) {
            if ( ! isset( $_FILES['import'] ) ) {
                return new WP_Error( 'masvideos_video_csv_importer_upload_file_empty', __( 'File is empty. Please upload something more substantial. This error could also be caused by uploads being disabled in your php.ini or by post_max_size being defined as smaller than upload_max_filesize in php.ini.', 'masvideos' ) );
            }

            if ( ! self::is_file_valid_csv( masvideos_clean( wp_unslash( $_FILES['import']['name'] ) ), false ) ) {
                return new WP_Error( 'masvideos_video_csv_importer_upload_file_invalid', __( 'Invalid file type. The importer supports CSV and TXT file formats.', 'masvideos' ) );
            }
            ...
            if ( isset( $upload['error'] ) ) {
                return new WP_Error( 'masvideos_video_csv_importer_upload_error', $upload['error'] );
            }
        ...
        return new WP_Error( 'masvideos_video_csv_importer_upload_invalid_file', __( 'Please upload or provide the link to a valid CSV file.', 'masvideos' ) );
    }
    ...
    protected function mapping_form() {
        ...
        if ( empty( $sample ) ) {
            $this->add_error(
                ...
                array(
                    array(
                        'url'   => admin_url( 'edit.php?post_type=video&page=video_importer' ),
    ...
    public function import() {
        ...
        // phpcs:disable WordPress.CSRF.NonceVerification.NoNonceVerification -- Nonce already verified in MasVideos_Admin_Importers::do_ajax_video_import()
        if ( ! empty( $_POST['map_from'] ) && ! empty( $_POST['map_to'] ) ) {
            ...
            update_user_option( get_current_user_id(), 'masvideos_video_import_mapping', $mapping_to );
        } else {
        ...
        wp_localize_script(
            'masvideos-video-import', 'masvideos_video_import_params', array(
                'import_nonce'    => wp_create_nonce( 'masvideos-video-import' ),
                'mapping'         => array(
                    'from' => $mapping_from,
                    'to'   => $mapping_to,
                ),
        ...
        wp_enqueue_script( 'masvideos-video-import' );
    ...
    protected function done() {
        ...
        $imported   = isset( $_GET['videos-imported'] ) ? absint( $_GET['videos-imported'] ) : 0;
        $updated    = isset( $_GET['videos-updated'] ) ? absint( $_GET['videos-updated'] ) : 0;
        $failed     = isset( $_GET['videos-failed'] ) ? absint( $_GET['videos-failed'] ) : 0;
        $skipped    = isset( $_GET['videos-skipped'] ) ? absint( $_GET['videos-skipped'] ) : 0;
        $errors     = array_filter( (array) get_user_option( 'video_import_error_log' ) );
        // phpcs:enable
    ...
    protected function auto_map_columns( $raw_headers, $num_indexes = true ) {
        ...
        $default_columns = $this->normalize_columns_names(
            apply_filters(
                'masvideos_csv_video_import_mapping_default_columns', array(
                    __( 'ID', 'masvideos' )                        => 'id',
                    ...
                    __( 'Video Choice', 'masvideos' )              => 'video_choice',
                    __( 'Video Attachment', 'masvideos' )          => 'video_attachment_id',
                    __( 'Video Embed Content', 'masvideos' )       => 'video_embed_content',
                    __( 'Video Link', 'masvideos' )                => 'video_url_link',
                )
            )
        );
        ...
        $special_columns = $this->get_special_columns(
            $this->normalize_columns_names(
                apply_filters(
                    'masvideos_csv_video_import_mapping_special_columns',
                    array(
        ...
        return apply_filters( 'masvideos_csv_video_import_mapped_columns', $headers, $raw_headers );
    }
    ...
    public function auto_map_user_preferences( $headers ) {
        $mapping_preferences = get_user_option( 'masvideos_video_import_mapping' );
    ...
    protected function get_mapping_options( $item = '' ) {
        ...
        $options        = array(
            ...
            'video_choice'           => __( 'Video Choice', 'masvideos' ),
            'video_attachment_id'    => __( 'Video Attachment', 'masvideos' ),
            'video_embed_content'    => __( 'Video Embed Content', 'masvideos' ),
            'video_url_link'         => __( 'Video Link', 'masvideos' ),
            'attributes'             => array(
        ...
        return apply_filters( 'masvideos_csv_video_import_mapping_options', $options, $item );
    }
}
