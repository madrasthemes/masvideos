...
<div data-taxonomy="<?php echo esc_attr( $attribute->get_taxonomy() ); ?>" class="masvideos_attribute masvideos-metabox closed <?php echo esc_attr( implode( ' ', $metabox_class ) ); ?>" rel="<?php echo esc_attr( $attribute->get_position() ); ?>">
    <h3>
        ...
        <strong class="attribute_name"><?php echo masvideos_episode_attribute_label( $attribute->get_name() ); ?></strong>
    </h3>
    <div class="masvideos_attribute_data masvideos-metabox-content">
        <table cellpadding="0" cellspacing="0">
            <tbody>
                <tr>
                    <td class="attribute_name">
                        ...
                        <?php if ( $attribute->is_taxonomy() ) : ?>
                            <strong><?php echo masvideos_episode_attribute_label( $attribute->get_name() ); ?></strong>
                        ...
                        if ( $attribute->is_taxonomy() && $attribute_taxonomy = $attribute->get_taxonomy_object() ) {
                            ...
                            if ( 'select' === $attribute_taxonomy->attribute_type ) {
                                ...
                                <select multiple="multiple" data-placeholder="<?php esc_attr_e( 'Select terms', 'masvideos' ); ?>" class="multiselect attribute_values masvideos-enhanced-select" name="attribute_values[<?php echo esc_attr( $i ); ?>][]">
                                    ...
                                    $all_terms = get_terms( $attribute->get_taxonomy(), apply_filters( 'masvideos_episode_attribute_terms', $args ) );
                                    if ( $all_terms ) {
                                        foreach ( $all_terms as $term ) {
                                            $options = $attribute->get_options();
                                            $options = ! empty( $options ) ? $options : array();
                                            echo '<option value="' . esc_attr( $term->term_id ) . '"' . masvideos_selected( $term->term_id, $options ) . '>' . esc_attr( apply_filters( 'masvideos_episode_attribute_term_name', $term->name, $term ) ) . '</option>';
                                        }
                                    }
                                    ?>
                                </select>
                            ...
                            do_action( 'masvideos_episode_option_terms', $attribute_taxonomy, $i );
                        } else {
                ...
                <tr>
                    <td>
                        <label><input type="checkbox" class="checkbox" <?php checked( $attribute->get_visible(), true ); ?> name="attribute_visibility[<?php echo esc_attr( $i ); ?>]" value="1" /> <?php esc_html_e( 'Visible on the episode page', 'masvideos' ); ?></label>
                    </td>
                </tr>
                <?php do_action( 'masvideos_after_episode_attribute_settings', $attribute, $i ); ?>
            </tbody>
        </table>
    </div>
</div>
