...
/**
 * MasVideos_Settings_API class.
 */
abstract class MasVideos_Settings_API {
	...
	public function get_form_fields() {
		return apply_filters( 'masvideos_settings_api_form_fields_' . $this->id, array_map( array( $this, 'set_defaults' ), $this->form_fields ) );
	}
	...
	public function update_option( $key, $value = '' ) {
		...
		return update_option( $this->get_option_key(), apply_filters( 'masvideos_settings_api_sanitized_fields_' . $this->id, $this->settings ), 'yes' );
	}
	...
	public function get_tooltip_html( $data ) {
		...
		return $tip ? masvideos_help_tip( $tip, true ) : '';
	}
			...
			<td class="forminp">
				<fieldset>
					...
					<?php if ( $data['select_buttons'] ) : ?>
						<br/><a class="select_all button" href="#"><?php esc_html_e( 'Select all', 'masvideos' ); ?></a> <a class="select_none button" href="#"><?php esc_html_e( 'Select none', 'masvideos' ); ?></a>
					<?php endif; ?>
				</fieldset>
			</td>
		</tr>
	...
	public function generate_title_html( $key, $data ) {
		...
		?>
			...
			<h3 class="masvideos-settings-sub-title <?php echo esc_attr( $data['class'] ); ?>" id="<?php echo esc_attr( $field_key ); ?>"><?php echo wp_kses_post( $data['title'] ); ?></h3>
	...
	public function validate_price_field( $key, $value ) {
		$value = is_null( $value ) ? '' : $value;
		return ( '' === $value ) ? '' : masvideos_format_decimal( trim( stripslashes( $value ) ) );
	}
	...
	public function validate_select_field( $key, $value ) {
		$value = is_null( $value ) ? '' : $value;
		return masvideos_clean( stripslashes( $value ) );
	}
	...
	public function validate_multiselect_field( $key, $value ) {
		return is_array( $value ) ? array_map( 'masvideos_clean', array_map( 'stripslashes', $value ) ) : '';
	}
}
