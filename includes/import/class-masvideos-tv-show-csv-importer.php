<?php
/**
 * MasVideos TV Show CSV importer
 *
 ...
 */
if ( ! class_exists( 'MasVideos_TV_Show_Importer', false ) ) {
    include_once dirname( __FILE__ ) . '/abstract-masvideos-tv-show-importer.php';
}

/**
 * MasVideos_TV_Show_CSV_Importer Class.
 */
class MasVideos_TV_Show_CSV_Importer extends MasVideos_TV_Show_Importer {
    ...
    protected function read_file() {
        if ( ! MasVideos_TV_Show_CSV_Importer_Controller::is_file_valid_csv( $this->file ) ) {
    ...
    /**
     * Parse relative field and return tv show ID.
     *
     * Handles `id:xx` and SKUs.
     *
     * If mapping to an id: and the tv show ID does not exist, this link is not
     * valid.
     *
     * If mapping to a SKU and the tv show ID does not exist, a temporary object
    ...
    public function parse_relative_field( $value ) {
        ...
        if ( preg_match( '/^id:(\d+)$/', $value, $matches ) ) {
            ...
            // See if the given ID maps to a valid tv show allready.
            $existing_id = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_type IN ( 'tv_show' ) AND ID = %d;", $id ) ); // WPCS: db call ok, cache ok.

            if ( $existing_id ) {
                return absint( $existing_id );
            }

            // If we're not updating existing posts, we may need a placeholder tv show to map to.
            if ( ! $this->params['update_existing'] ) {
                $tv_show = new MasVideos_TV_Show();
                $tv_show->set_name( 'Import placeholder for ' . $id );
                $tv_show->set_status( 'importing' );
                $tv_show->add_meta_data( '_original_id', $id, true );
                $id = $tv_show->save();
            }
        ...
        try {
            $tv_show = new MasVideos_TV_Show();
            $tv_show->set_name( 'Import placeholder for ' . $value );
            $tv_show->set_status( 'importing' );
            $id = $tv_show->save();
     ...
     * Parse the ID field.
     *
     * If we're not doing an update, create a placeholder tv show so mapping works
    ...
    /**
     * Parse relative comma-delineated field and return tv show ID.
     *
     * @param string $value Field value.
     * @return array
     */
    ...
    public function parse_genres_field( $value ) {
        ...
        foreach ( $row_terms as $row_term ) {
            ...
            foreach ( $_terms as $index => $_term ) {
                ...
                $term = term_exists( $_term, 'tv_show_genre', $parent );

                if ( is_array( $term ) ) {
                    $term_id = $term['term_id'];
                    // Don't allow users without capabilities to create new genres.
                } elseif ( ! current_user_can( 'manage_tv_show_terms' ) ) {
                    break;
                } else {
                    $term = wp_insert_term( $_term, 'tv_show_genre', array( 'parent' => intval( $parent ) ) );
    ...
    public function parse_tags_field( $value ) {
        ...
        foreach ( $names as $name ) {
            $term = get_term_by( 'name', $name, 'tv_show_tag' );

            if ( ! $term || is_wp_error( $term ) ) {
                $term = (object) wp_insert_term( $name, 'tv_show_tag' );
            }
    ...
    protected function get_formating_callback() {
        ...
        return apply_filters( 'masvideos_tv_show_importer_formatting_callbacks', $callbacks, $this );
    }
    ...
    /**
     * Expand special and internal data into the correct formats for the tv show CRUD.
     *
     * @param  array $data Data to import.
     * @return array
     */
    protected function expand_data( $data ) {
        $data = apply_filters( 'masvideos_tv_show_importer_pre_expand_data', $data );
        ...
        foreach ( $data as $key => $value ) {
            if ( $this->starts_with( $key, 'seasons:name' ) ) {
                if ( ! empty( $value ) ) {
                    $seasons[ str_replace( 'seasons:name', '', $key ) ]['name'] = $value;
                }
                unset( $data[ $key ] );

            } elseif ( $this->starts_with( $key, 'seasons:image_id' ) ) {
                if ( ! empty( $value ) ) {
                    $seasons[ str_replace( 'seasons:image_id', '', $key ) ]['image_id'] = array_shift( $value );
                }
                unset( $data[ $key ] );

            } elseif ( $this->starts_with( $key, 'seasons:episodes' ) ) {
                if ( ! empty( $value ) ) {
            ...
            } elseif ( $this->starts_with( $key, 'seasons:position' ) ) {
                if ( ! empty( $value ) ) {
                    $seasons[ str_replace( 'seasons:position', '', $key ) ]['position'] = $value;
                }
                unset( $data[ $key ] );
    ...
    protected function set_parsed_data() {
        ...
        foreach ( $this->raw_data as $row_index => $row ) {
            ...
            do_action( 'masvideos_tv_show_importer_before_set_parsed_data', $row, $mapped_keys );
            ...
            $this->parsed_data[] = apply_filters( 'masvideos_tv_show_importer_parsed_data', $this->expand_data( $data ), $this );
        }
    }
     ...
     * Process importer.
     *
     * Do not import tv shows with IDs or SKUs that already exist if option
     * update existing is false, and likewise, if updating tv shows, do not
    ...
    public function import() {
        ...
        foreach ( $this->parsed_data as $parsed_data_key => $parsed_data ) {
            do_action( 'masvideos_tv_show_import_before_import', $parsed_data );
            ...
            if ( $id ) {
                ...
                $id_exists = $tv_show && 'importing' !== $tv_show->get_status();
            }
