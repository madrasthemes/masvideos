 ...
 */
class MasVideos_Shortcode_TV_Shows {
     ...
     */
    public function __construct( $attributes = array(), $type = 'tv_shows' ) {
        $this->type       = $type;
    ...
    protected function parse_attributes( $attributes ) {
        ...
        if ( ! absint( $attributes['columns'] ) ) {
            $attributes['columns'] = masvideos_get_default_tv_shows_per_row();
        }
    ...
    protected function parse_query_args() {
        ...
        if ( masvideos_string_to_bool( $this->attributes['paginate'] ) ) {
            $this->attributes['page'] = absint( empty( $_GET['tv_show-page'] ) ? 1 : $_GET['tv_show-page'] ); // WPCS: input var ok, CSRF ok.
        }
        ...
        $ordering_args                = MasVideos()->tv_show_query->get_catalog_ordering_args( $query_args['orderby'], $query_args['order'] );
        ...
        $query_args['meta_query']     = MasVideos()->tv_show_query->get_meta_query();
        ...
        $query_args = apply_filters( 'masvideos_shortcode_tv_shows_query', $query_args, $this->attributes, $this->type );
    ...
    protected function set_attributes_query_args( &$query_args ) {
        if ( ! empty( $this->attributes['attribute'] ) || ! empty( $this->attributes['terms'] ) ) {
            $taxonomy = strstr( $this->attributes['attribute'], 'tv_show_' ) ? sanitize_title( $this->attributes['attribute'] ) : 'tv_show_' . sanitize_title( $this->attributes['attribute'] );
            ...
            // If no terms were specified get all tv_shows that are in the attribute taxonomy.
            if ( ! $terms ) {
                $terms = get_terms(
                    array(
                        'taxonomy' => $taxonomy,
        ...
        if ( ! empty( $this->attributes['genre'] ) ) {
            ...
            if ( is_numeric( $genres[0] ) ) {
                ...
                foreach ( $genres as $genre ) {
                    $the_genre = get_term_by( 'slug', $genre, 'tv_show_genre' );
                    if ( false !== $the_genre ) {
            ...
            $query_args['tax_query'][] = array(
                 ...
                 * as only tv shows belonging to all the children genres would be selected.
                 */
    ...
    protected function set_tags_query_args( &$query_args ) {
        if ( ! empty( $this->attributes['tag'] ) ) {
            ...
            if ( is_numeric( $tags[0] ) ) {
                ...
                // Check numeric slugs.
                foreach ( $tags as $tag ) {
                    $the_tag = get_term_by( 'slug', $tag, 'tv_show_tag' );
                    if ( false !== $the_tag ) {
    ...
    protected function set_visibility_featured_query_args( &$query_args ) {
        // @codingStandardsIgnoreStart
        $query_args['tax_query'] = array_merge( $query_args['tax_query'], MasVideos()->tv_show_query->get_tax_query() );
        // @codingStandardsIgnoreEnd
    ...
    protected function get_wrapper_classes( $columns ) {
        $classes = array( 'masvideos', 'masvideos-tv-shows' );
    ...
    protected function get_transient_name() {
        $transient_name = 'masvideos_tv_show_loop' . substr( md5( wp_json_encode( $this->query_args ) . $this->type ), 28 );
        ...
        if ( 'rand' === $this->query_args['orderby'] ) {
            ...
            $rand_index      = rand( 0, max( 1, absint( apply_filters( 'masvideos_tv_show_query_max_rand_cache_count', 5 ) ) ) );
            $transient_name .= $rand_index;
        }

        $transient_name .= MasVideos_Cache_Helper::get_transient_version( 'tv_show_query' );
    ...
    protected function get_query_results() {
        ...
        if ( false === $results ) {
            if ( 'top_rated_tv_shows' === $this->type ) {
        ...
        MasVideos()->tv_show_query->remove_ordering_args();
        return $results;
    }

    /**
     * Loop over found tv_shows.
     *
     * @since  1.0.0
     * @return string
     */
    protected function tv_show_loop() {
