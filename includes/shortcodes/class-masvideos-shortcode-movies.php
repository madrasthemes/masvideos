...
/**
 * Movies shortcode class.
 */
class MasVideos_Shortcode_Movies {
     ...
     */
    public function __construct( $attributes = array(), $type = 'movies' ) {
        $this->type       = $type;
    ...
    protected function parse_attributes( $attributes ) {
        ...
        if ( ! absint( $attributes['columns'] ) ) {
            $attributes['columns'] = masvideos_get_default_movies_per_row();
        }
    ...
    protected function parse_query_args() {
        ...
        if ( masvideos_string_to_bool( $this->attributes['paginate'] ) ) {
            $this->attributes['page'] = absint( empty( $_GET['movie-page'] ) ? 1 : $_GET['movie-page'] ); // WPCS: input var ok, CSRF ok.
        }
        ...
        $ordering_args                = MasVideos()->movie_query->get_catalog_ordering_args( $query_args['orderby'], $query_args['order'] );
        ...
        $query_args['meta_query']     = MasVideos()->movie_query->get_meta_query();
        ...
        $query_args = apply_filters( 'masvideos_shortcode_movies_query', $query_args, $this->attributes, $this->type );
    ...
    protected function set_attributes_query_args( &$query_args ) {
        if ( ! empty( $this->attributes['attribute'] ) || ! empty( $this->attributes['terms'] ) ) {
            $taxonomy = strstr( $this->attributes['attribute'], 'movie_' ) ? sanitize_title( $this->attributes['attribute'] ) : 'movie_' . sanitize_title( $this->attributes['attribute'] );
            ...
            // If no terms were specified get all movies that are in the attribute taxonomy.
            if ( ! $terms ) {
                $terms = get_terms(
                    array(
                        'taxonomy' => $taxonomy,
        ...
        if ( ! empty( $this->attributes['genre'] ) ) {
            ...
            if ( is_numeric( $genres[0] ) ) {
                ...
                foreach ( $genres as $genre ) {
                    $the_genre = get_term_by( 'slug', $genre, 'movie_genre' );
                    if ( false !== $the_genre ) {
            ...
            $query_args['tax_query'][] = array(
                 ...
                 * as only movies belonging to all the children genres would be selected.
                 */
    ...
    protected function set_visibility_featured_query_args( &$query_args ) {
        // @codingStandardsIgnoreStart
        $query_args['tax_query'] = array_merge( $query_args['tax_query'], MasVideos()->movie_query->get_tax_query() );
        // @codingStandardsIgnoreEnd
    ...
    protected function get_wrapper_classes( $columns ) {
        $classes = array( 'masvideos', 'masvideos-movies' );
    ...
    protected function get_transient_name() {
        $transient_name = 'masvideos_movie_loop' . substr( md5( wp_json_encode( $this->query_args ) . $this->type ), 28 );
        ...
        if ( 'rand' === $this->query_args['orderby'] ) {
            ...
            $rand_index      = rand( 0, max( 1, absint( apply_filters( 'masvideos_movie_query_max_rand_cache_count', 5 ) ) ) );
            $transient_name .= $rand_index;
        }

        $transient_name .= MasVideos_Cache_Helper::get_transient_version( 'movie_query' );
    ...
    protected function get_query_results() {
        ...
        MasVideos()->movie_query->remove_ordering_args();
        return $results;
    }

    /**
     * Loop over found movies.
     *
     * @since  1.0.0
     * @return string
     */
    protected function movie_loop() {
        ...
        if ( $movies && $movies->ids ) {
            ...
            // Fire standard movies loop hooks when paginating results so we can show result counts and so on.
            if ( masvideos_string_to_bool( $this->attributes['paginate'] ) ) {
