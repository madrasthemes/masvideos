...
/**
 * Movie Playlists shortcode class.
 */
class MasVideos_Shortcode_Movie_Playlists {
     ...
     * @var   string
     */
    protected $type = 'movie_playlists';
     ...
     */
    public function __construct( $attributes = array(), $type = 'movie_playlists' ) {
        $this->type       = $type;
    ...
    public function get_content() {
        return $this->movie_playlist_loop();
    }
    ...
    protected function parse_query_args() {
        ...
        if ( masvideos_string_to_bool( $this->attributes['paginate'] ) ) {
            $this->attributes['page'] = absint( empty( $_GET['movie_playlist-page'] ) ? 1 : $_GET['movie_playlist-page'] ); // WPCS: input var ok, CSRF ok.
        }
        ...
        $query_args = apply_filters( 'masvideos_shortcode_movie_playlists_query', $query_args, $this->attributes, $this->type );
    ...
    /**
     * Set movie_playlist as visible when quering for hidden movie_playlists.
     *
     * @since  1.0.0
     * @param  bool $visibility Movie Playlist visibility.
     * @return bool
     */
    public function set_movie_playlist_as_visible( $visibility ) {
    ...
    protected function get_wrapper_classes( $columns ) {
        $classes = array( 'masvideos', 'masvideos-movie-playlists' );
    ...
    protected function get_transient_name() {
        $transient_name = 'masvideos_movie_playlist_loop' . substr( md5( wp_json_encode( $this->query_args ) . $this->type ), 28 );
        ...
        if ( 'rand' === $this->query_args['orderby'] ) {
            ...
            $rand_index      = rand( 0, max( 1, absint( apply_filters( 'masvideos_movie_playlist_query_max_rand_cache_count', 5 ) ) ) );
            $transient_name .= $rand_index;
        }

        $transient_name .= MasVideos_Cache_Helper::get_transient_version( 'movie_playlist_query' );
    ...
    protected function get_query_results() {
        ...
        if ( false === $results ) {
            if ( 'top_rated_movie_playlists' === $this->type ) {
    ...
    /**
     * Loop over found movie_playlists.
     *
     * @since  1.0.0
     * @return string
     */
    protected function movie_playlist_loop() {
        $columns  = absint( $this->attributes['columns'] );
        $classes  = $this->get_wrapper_classes( $columns );
        $movie_playlists = $this->get_query_results();

        ob_start();

        if ( $movie_playlists && $movie_playlists->ids ) {
            // Prime caches to reduce future queries.
            if ( is_callable( '_prime_post_caches' ) ) {
                _prime_post_caches( $movie_playlists->ids );
            }
            ...
            masvideos_setup_movie_playlists_loop(
                array(
                    ...
                    'total'        => $movie_playlists->total,
                    'total_pages'  => $movie_playlists->total_pages,
                    'per_page'     => $movie_playlists->per_page,
                    'current_page' => $movie_playlists->current_page,
                )
            );
            ...
            // Fire standard movie_playlist loop hooks when paginating results so we can show result counts and so on.
            if ( masvideos_string_to_bool( $this->attributes['paginate'] ) ) {
                do_action( 'masvideos_before_movie_playlists_loop' );
            }

            masvideos_movie_playlist_loop_start();

            if ( masvideos_get_movie_playlists_loop_prop( 'total' ) ) {
                foreach ( $movie_playlists->ids as $movie_playlist_id ) {
                    $GLOBALS['post'] = get_post( $movie_playlist_id ); // WPCS: override ok.
                    setup_postdata( $GLOBALS['post'] );

                    // Set custom movie_playlist visibility when quering hidden movie_playlists.
                    add_action( 'masvideos_movie_playlist_is_visible', array( $this, 'set_movie_playlist_as_visible' ) );

                    // Render movie_playlist template.
                    if( ! empty( $this->attributes['template'] ) ) {
                        masvideos_get_template_part( $this->attributes['template'] );
                    } else {
                        masvideos_get_template_part( 'content', 'movie-playlist' );
                    }

                    // Restore movie_playlist visibility.
                    remove_action( 'masvideos_movie_playlist_is_visible', array( $this, 'set_movie_playlist_as_visible' ) );
                }
            }

            $GLOBALS['post'] = $original_post; // WPCS: override ok.
            masvideos_movie_playlist_loop_end();

            // Fire standard movie_playlist loop hooks when paginating results so we can show result counts and so on.
            if ( masvideos_string_to_bool( $this->attributes['paginate'] ) ) {
                do_action( 'masvideos_after_movie_playlists_loop' );
            }

            do_action( "masvideos_shortcode_after_{$this->type}_loop", $this->attributes );

            wp_reset_postdata();
            masvideos_reset_movie_playlists_loop();
        } else {
