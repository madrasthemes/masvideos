 ...
 * Genres widget class.
 */
class MasVideos_Widget_Tv_Shows_Genres extends MasVideos_Widget {
	...
	public function __construct() {
		...
		$this->widget_name        = __( 'Filter TV Shows by Genres', 'masvideos' );
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => __( 'TV Shows genres', 'masvideos' ),
	...
	public function widget( $args, $instance ) {
		...
		if ( is_tax( 'tv_show_genre' ) ) {
			$this->current_genre   = $wp_query->queried_object;
			$this->genre_ancestors = get_ancestors( $this->current_genre->term_id, 'tv_show_genre' );

		} elseif ( is_singular( 'tv_show' ) ) {
			$terms = masvideos_get_tv_show_terms(
				$post->ID, 'tv_show_genre', apply_filters(
					'masvideos_tv_show_genres_widget_terms_args', array(
						'orderby' => 'parent',
						'order'   => 'DESC',
					)
				)
			...
			if ( $terms ) {
				$main_term           = apply_filters( 'masvideos_tv_show_genres_widget_main_term', $terms[0], $terms );
				$this->current_genre   = $main_term;
				$this->genre_ancestors = get_ancestors( $main_term->term_id, 'tv_show_genre' );
			}
		}
		...
		if ( $dropdown ) {
			masvideos_tv_show_dropdown_genres(
				apply_filters(
					'masvideos_tv_show_genres_widget_dropdown_args', wp_parse_args(
						$dropdown_args, array(
							'show_count'         => $count,
							'hierarchical'       => $hierarchical,
			...
			masvideos_enqueue_js(
				"
				jQuery( '.dropdown_tv_show_genre' ).change( function() {
					if ( jQuery(this).val() != '' ) {
						var this_page = '';
						var home_url  = '" . esc_js( home_url( '/' ) ) . "';
						if ( home_url.indexOf( '?' ) > 0 ) {
							this_page = home_url + '&tv_show_genre=' + jQuery(this).val();
						} else {
							this_page = home_url + '?tv_show_genre=' + jQuery(this).val();
						}
						location.href = this_page;
					} else {
						location.href = '" . esc_js( masvideos_get_page_permalink( 'tv_shows' ) ) . "';
					}
				});

				if ( jQuery().selectWoo ) {
					var masvideos_tv_show_genre_select = function() {
						jQuery( '.dropdown_tv_show_genre' ).selectWoo( {
					...
					masvideos_tv_show_genre_select();
				}
			"
			);
		} else {
			include_once MasVideos()->plugin_path() . '/includes/walkers/class-masvideos-tv-shows-genre-list-walker.php';

			$list_args['walker']                     = new MasVideos_TV_Show_Genre_List_Walker();
			$list_args['title_li']                   = '';
			$list_args['pad_counts']                 = 1;
			$list_args['show_option_none']           = __( 'No TV Shows genres exist.', 'masvideos' );
			...
			echo '<ul class="tv-shows-genres">';

			wp_list_categories( apply_filters( 'masvideos_tv_shows_genres_widget_args', $list_args ) );
