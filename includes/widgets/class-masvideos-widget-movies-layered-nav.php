 ...
 */
class MasVideos_Widget_Movies_Layered_Nav extends MasVideos_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'masvideos widget_layered_nav masvideos-widget-movies-layered-nav';
        $this->widget_description = esc_html__( 'Display a list of attributes to filter movies in your site.', 'masvideos' );
        $this->widget_id          = 'masvideos_widget_movies_layered_nav';
        $this->widget_name        = esc_html__( 'MAS Videos Filter Movies by Attribute', 'masvideos' );
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            ...
            'attribute'    => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => __( 'Attribute', 'masvideos' ),
                'options' => $attribute_array,
            ),
            'display_type' => array(
                'type'    => 'select',
                'std'     => 'list',
                'label'   => __( 'Display type', 'masvideos' ),
                'options' => array(
                    'list'     => __( 'List', 'masvideos' ),
                    'dropdown' => __( 'Dropdown', 'masvideos' ),
                ),
            ),
            'query_type'   => array(
                'type'    => 'select',
    ...
    public function widget( $args, $instance ) {
        ...
        $_chosen_attributes = MasVideos_Movies_Query::get_layered_nav_chosen_attributes();
        $taxonomy           = isset( $instance['attribute'] ) ? masvideos_attribute_taxonomy_name( $this->get_post_type(), $instance['attribute'] ) : $this->settings['attribute']['std'];
    ...
    protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
        ...
        if ( $taxonomy !== $this->get_current_taxonomy() ) {
            $term_counts          = $this->get_filtered_term_movie_counts( wp_list_pluck( $terms, 'term_id' ), $taxonomy, $query_type );
            $_chosen_attributes   = MasVideos_Movies_Query::get_layered_nav_chosen_attributes();
            $taxonomy_filter_name = str_replace( $this->get_post_type() . '_', '', $taxonomy );
            $taxonomy_label       = masvideos_movie_attribute_label( $taxonomy );

            /* translators: %s: taxonomy name */
            $any_label      = apply_filters( 'masvideos_layered_nav_any_label', sprintf( __( 'Any %s', 'masvideos' ), $taxonomy_label ), $taxonomy_label, $taxonomy );
            ...
            echo '<form method="get" action="' . esc_url( $form_action ) . '" class="masvideos-widget-movies-layered-nav-dropdown">';
            echo '<select class="masvideos-widget-movies-layered-nav-dropdown dropdown_layered_nav_' . esc_attr( $taxonomy_filter_name ) . '"' . ( $multiple ? 'multiple="multiple"' : '' ) . '>';
            ...
            if ( $multiple ) {
                echo '<button class="masvideos-widget-movies-layered-nav-dropdown__submit" type="submit" value="' . esc_attr__( 'Apply', 'masvideos' ) . '">' . esc_html__( 'Apply', 'masvideos' ) . '</button>';
            }
            ...
            echo masvideos_query_string_form_fields( null, array( 'filter_' . $taxonomy_filter_name, 'query_type_' . $taxonomy_filter_name ), '', true ); // @codingStandardsIgnoreLine
            echo '</form>';
    ...
    /**
     * Count movies within certain terms, taking the main WP query into consideration.
     *
     * This query allows counts to be generated based on the viewed movies, not all movies.
     *
     ...
     * @return array
     */
    protected function get_filtered_term_movie_counts( $term_ids, $taxonomy, $query_type ) {
        global $wpdb;
        ...
        $query['where'] = "
            WHERE {$wpdb->posts}.post_type IN ( 'movie' )
        ...
        $query             = apply_filters( 'masvideos_get_filtered_term_movie_counts_query', $query );
        $query             = implode( ' ', $query );
    ...
    protected function layered_nav_list( $terms, $taxonomy, $query_type ) {
        // List display.
        echo '<ul class="masvideos-widget-movies-layered-nav-list">';
        ...
        foreach ( $terms as $term ) {
            ...
            echo '<li class="masvideos-widget-movies-layered-nav-list__item masvideos-layered-nav-term ' . ( $option_is_set ? 'masvideos-widget-movies-layered-nav-list__item--chosen chosen' : '' ) . '">';
