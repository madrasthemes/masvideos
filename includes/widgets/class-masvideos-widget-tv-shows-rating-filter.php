 ...
 */
class MasVideos_Widget_Tv_Shows_Rating_Filter extends MasVideos_Widget {
	...
	public function __construct() {
		...
		$this->widget_name        = __( 'MAS Videos Filter TV Shows by Rating', 'masvideos' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
	 ...
	 * @return int
	 */
	protected function get_filtered_tv_show_count( $rating ) {
		global $wpdb;
		...
		$tax_query[]              = array(
			...
			'terms'         => $tv_show_visibility_terms[ 'rated-' . $rating ],
			'operator'      => 'IN',
			'rating_filter' => true,
		);
		...
		$sql .= " WHERE {$wpdb->posts}.post_type = 'tv_show' AND {$wpdb->posts}.post_status = 'publish' ";
	...
	public function widget( $args, $instance ) {
		...
		for ( $rating = 10; $rating >= 1; $rating-- ) {
			$count = $this->get_filtered_tv_show_count( $rating );
			if ( empty( $count ) ) {
				continue;
			}
			$found = true;
			...
			$link        = apply_filters( 'masvideos_tv_shows_rating_filter_link', $link_ratings ? add_query_arg( 'rating_filter', $link_ratings ) : remove_query_arg( 'rating_filter' ) );
			$rating_html = masvideos_get_star_rating_html( $rating );
			$count_html  = esc_html( apply_filters( 'masvideos_tv_shows_rating_filter_count', "({$count})", $count, $rating ) );
